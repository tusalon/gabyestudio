...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = 'b7c59c75-823b-46b8-bfb4-9abbbf504abc'; // ID de gaby.estudio
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'gaby.estudio';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '58450883';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'gaby001401@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a gaby.estudio!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'gaby-estudio';
};
...
console.log('✅ config-negocio.js listo para gaby.estudio');
